  ...
  if (table_idx < 0) {          // unknown special key, may output t_xx
    if (IS_SPECIAL(c)) {
      ...
      if (len == 1 && vim_isprintc(c)) {
        if (data) {
          if ('A' <= c && c <= 'Z') {
            data->_small_key = (char)c + 32;
            data->key = (String){ &data->_small_key, 1 };
            if (!is_ascii_ctrl) {
              data->modifiers |= MOD_MASK_SHIFT;
            }
            data->alt_key = (String){ &string[idx], (size_t)len };
          } else {
            data->alt_key = (String){ NULL, 0 };
        ...
        if (data) {
          data->_small_key = (char)c;
          data->key = (String){ &data->_small_key, 1 };
          data->alt_key = (String){ &string[idx], strlen(s) };
        }
